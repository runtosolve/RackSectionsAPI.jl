...
function get_deformed_shape_extents(model, eig, deformation_scale)
    ...
    t = model.elem[:, 4]
    n = fill(5, length(t))
    cross_section_coords, Δ_nodes, figure_max_dim_range = CUFSM.Show.cross_section_mode_shape_info(model.elem, model.node, mode, n, deformation_scale)
...
function signature_curve(model, all_figures, api_figure_options)
    ...
    ax, figure = CUFSM.Show.signature_curve(model, eig, options)
    push!(all_figures, figure)
...
function mode_shape(model, t, t_elements, all_figures, api_figure_options)
    ...
    options = CUFSM.Show.ModeShapeOptions(drawing_size, thickness_scale, backgroundcolor, linecolor, linestyle, joinstyle)

    ax, figure = CUFSM.Show.minimum_mode_shape(model, eig, t_elements, deformation_scale, options)
    push!(all_figures, figure)
